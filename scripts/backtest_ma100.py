...
100일 이동평균선의 기울기 전환과 터치 반등을 이용한 롱/숏 전략.

시그널 A: 기울기 전환 (Slope Reversal)
  - MA100 기울기 하락→상승 전환 → 롱
  - MA100 기울기 상승→하락 전환 → 숏

시그널 B: MA100 터치 반등/저항 (Touch Bounce)
  - 기울기 상향 + 저가가 MA100 터치 + 종가 MA100 위 → 롱 (지지 반등)
  - 기울기 하향 + 고가가 MA100 터치 + 종가 MA100 아래 → 숏 (저항 하락)

사용법:
    python scripts/backtest_ma100.py
    python scripts/backtest_ma100.py --initial 1000 --start 2024-06-01 --end 2026-01-31
"""
...
MA100_PARAMS = {
    ...
    # ── 분할매수 (DCA) ──
    "dca_ratios": [1],           # 분할매수 비율 ([1]=단일진입, [1,1]=1:1, [1,1,2]=1:1:2)
    "dca_interval_pct": 2.0,     # 분할매수 간격 (%) - 숏: 진입가 위로, 롱: 진입가 아래로
}
...
def load_all_data(
    ...
    min_candles = 100  # MA100 계산에 필요한 최소 캔들 수
    ...
    for i, symbol in enumerate(symbols):
        ...
        if timeframe == "1d":
            ...
            # 4h 데이터 직접 로드
            if "4h" in tfs:
                df = loader.load(symbol, "4h", start=start, end=end)
                if df is not None and len(df) >= min_candles:
                    loaded_native += 1
                else:
                    df = None
        ...
        if (i + 1) % 100 == 0 or (i + 1) == len(symbols):
            logger.info(f"데이터 로드: {i+1}/{len(symbols)} ({timeframe}: {loaded_native}, resample: {resampled}, skip: {skipped})")
...
class MA100Backtester:
    ...
    def _calc_qty(self, price: float) -> float:
        margin = self.balance * self.params["position_pct"]
    ...
    def _process_dca_fills(self, symbol: str, candle: pd.Series):
        """캔들 내에서 DCA 주문 체결 처리. SL/TP 체크 전에 호출."""
        ...
        for dca in pending:
            # 숏: 가격이 DCA 레벨까지 올라가면 체결
            # 롱: 가격이 DCA 레벨까지 내려가면 체결
    ...
    def _check_exit_candle(self, symbol: str, candle: pd.Series, dt: pd.Timestamp):
        ...
        """
        # DCA 체결 먼저 처리 (SL보다 안쪽이므로 먼저 체결됨)
    ...
    def _precompute_signals(self, all_data: Dict[str, pd.DataFrame]) -> Dict[str, pd.DataFrame]:
        ...
        for idx, (symbol, df) in enumerate(all_data.items()):
            ...
            # NaN 처리
            for col in ["long_signal", "short_signal", "reversal_long", "reversal_short", "touch_long", "touch_short"]:
    ...
    def run(self, all_data: Dict[str, pd.DataFrame], start_dt: datetime, end_dt: datetime):
        ...
        logger.info(f"백테스트 구간: {len(all_ts)} 캔들 ({start_dt} ~ {end_dt})")
...
def print_report(bt: MA100Backtester, start_dt: datetime, end_dt: datetime):
    ...
    print()
    print("=" * 55)
    print("  MA100 Slope Strategy Backtest")
    print("=" * 55)
...
<title>MA100 Slope Strategy Backtest Report</title>
...
<h1>MA100 Slope Strategy Backtest</h1>
<p class="subtitle">{start_dt.strftime('%Y-%m-%d')} ~ {end_dt.strftime('%Y-%m-%d')} ({days} days) &nbsp;|&nbsp; {num_symbols} symbols &nbsp;|&nbsp; {timeframe} candles</p>
...
<div class="card">
  <div class="param-grid">
    ...
    <div class="param-item"><span class="param-label">DCA Ratios</span><span class="param-value">{":".join(str(r) for r in bt.params.get('dca_ratios', [1]))}</span></div>
    <div class="param-item"><span class="param-label">DCA Interval</span><span class="param-value">{bt.params.get('dca_interval_pct', 0)}%</span></div>
  </div>
</div>
...
document.addEventListener('keydown', function(e) {{ if (e.key === 'Escape') closeTradeChart(); }});
    ...
    out_path = Path("data/ma100_backtest_report.html")
...
def main():
    parser = argparse.ArgumentParser(description="MA100 Slope Strategy Backtest")
    ...
    parser.add_argument("--timeframe", default="1d", choices=["1d", "4h"], help="Candle timeframe (1d or 4h)")
    parser.add_argument("--dca-ratios", default=None, help="DCA ratios (e.g. '1:1', '1:1:2'). Default: no DCA")
    parser.add_argument("--dca-interval", type=float, default=2.0, help="DCA interval %% (default: 2.0)")
    ...
    # DCA 설정
    if args.dca_ratios:
        params["dca_ratios"] = [int(x) for x in args.dca_ratios.split(":")]
        params["dca_interval_pct"] = args.dca_interval

    max_pos = args.max_positions or params["max_positions"]
    tf = args.timeframe

    dca_desc = ":".join(str(r) for r in params["dca_ratios"])
    if len(params["dca_ratios"]) > 1:
        logger.info(f"=== MA100 Slope Backtest ({tf}) DCA {dca_desc} interval={params['dca_interval_pct']}%: {args.start} ~ {args.end} ===")
    else:
        logger.info(f"=== MA100 Slope Backtest ({tf}): {args.start} ~ {args.end} ===")

    # ── 데이터 로드 ──
    loader = DataLoader()

    # MA100 계산을 위한 여유 기간 (4h: 100캔들 ≈ 17일, 1d: 100캔들 = 100일)
    buffer_days = 30 if tf == "4h" else 150
    data_start = start_dt - timedelta(days=buffer_days)
    data_start_str = data_start.strftime("%Y-%m-%d")

    logger.info(f"데이터 로드 범위: {data_start_str} ~ {args.end} (MA100 계산 포함, {tf})")

    all_data = load_all_data(loader, timeframe=tf, start=data_start_str, end=args.end)
    ...
    html_path = generate_html_report(bt, start_dt, end_dt, precomputed, timeframe=tf)
