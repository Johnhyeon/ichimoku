"""
백테스트 옵션 1: Cross 조건 완화
- 최근 N개 캔들 내 크로스 발생 시 진입 허용
"""
...
# 옵션 1 파라미터: 최근 N개 캔들 내 크로스 허용
CROSS_LOOKBACK = 3  # 최근 3개 캔들 내 크로스 발생 시 진입
...
def run_backtest(
    ...
    """
    백테스트 실행 - 옵션 1: Cross 조건 완화 (SHORT 전용)
    """
    all_bars = []
    ...
    for symbol, df in all_data.items():
        ...
        # 크로스 히스토리 초기화
        cross_history[symbol] = []

        for idx, row in df.iterrows():
            # 크로스 발생 시 히스토리에 추가
            if row['tk_cross_down'] or row['kijun_cross_down']:
                cross_history[symbol].append(row['timestamp'])
    ...
    for t in sorted_times:
        ...
        if cash >= position_size and len(positions) < params['max_positions']:
            ...
            for sym, bar in bars.items():
                ...
                # === 숏 조건 (옵션 1: 최근 N개 캔들 내 크로스 허용) ===
                if bar['below_cloud'] and not bar['tenkan_above']:
                    # 현재 캔들 크로스 또는 최근 N개 캔들 내 크로스
                    has_recent_cross = bar['tk_cross_down'] or bar['kijun_cross_down']

                    if not has_recent_cross and sym in cross_history:
                        # 최근 N개 캔들(시간) 내 크로스 확인
                        lookback_time = pd.Timedelta(hours=4 * cross_lookback)
                        for cross_time in reversed(cross_history[sym]):
                            if t - cross_time <= lookback_time:
                                has_recent_cross = True
                                break
                            elif t - cross_time > lookback_time:
                                break

                    if not has_recent_cross:
                        continue
...
if __name__ == '__main__':
    ...
    print("=" * 70)
    print("옵션 1: Cross 조건 완화 (최근 3캔들 내 크로스 허용) - SHORT 전용")
    print("=" * 70)
    print(f"초기 자본: ${INITIAL_CAPITAL:,}")
    print(f"레버리지: {LEVERAGE}x")
    print(f"포지션 크기: {POSITION_PCT*100}%")
    print(f"Cross Lookback: {CROSS_LOOKBACK} 캔들")
    print("=" * 70)
